...
export class LunchRegistrationService {
  ...
  registerUserForLunch(
    registrationDto: LunchRegistrationDTO
  ): Observable<string> {
    return this.http.post('http://localhost:8080/api/lunch', registrationDto, {
      responseType: 'text',
    });
  }

  updateUserLunchRegistration(
    userId: number,
    registrationDto: LunchRegistrationDTO
  ): Observable<string> {
    return this.http.put<string>(`${this.apiUrl}/${userId}`, registrationDto, {
  ...
  updateLunchRegistration(
    userId: number,
    registrationDto: LunchRegistrationDTO
  ): Observable<any> {
    return this.http.put(`${this.apiUrl}/${userId}`, registrationDto);
  }
  updateLunchRegistrationForNextMonth(
    userId: number,
    registrationDto: LunchRegistrationDTO
  ): Observable<any> {
    return this.http.put<any>(
      `${this.apiUrl}/update-next-month/${userId}`,
      registrationDto
    );
  }
  ...
  getRegistrationCutoff(): Observable<{
    lastRegisterDay: string;
    lastRegisterTime: string;
  }> {
    return this.http.get<{ lastRegisterDay: string; lastRegisterTime: string }>(
      'http://localhost:8080/api/settings/registration-cutoff'
    );
  }
}
